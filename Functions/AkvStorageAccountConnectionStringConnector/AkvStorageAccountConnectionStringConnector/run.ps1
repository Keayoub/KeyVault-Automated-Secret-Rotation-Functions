<#
This PowerShell script is designed for Azure Functions that automatically handle the rotation and import of credentials (Azure Storage account connection strings) stored in Azure Key Vault (AKV) by responding to Azure Event Grid events.
...
# Function to retrieve the value of the active credential (account connection string) from the secret provider (Azure Storage).
# This function validates the input and retrieves the specified key from the Azure Storage account, then forms the corresponding connection string.
function Get-CredentialValue([string]$ActiveCredentialId, [string]$ProviderAddress) {
    ...
    # Select the subscription to operate on and retrieve the expected storage endpoint suffix.
    $null = Set-AzContext -SubscriptionId $subscriptionId
    $storageEndpointSuffix = (Get-AzContext).Environment.StorageEndpointSuffix

    # Retrieve the specified credential (account connection string) from the secret provider (Azure Storage).
    try {
        $accountKey = (Get-AzStorageAccountKey -ResourceGroupName $resourceGroupName -AccountName $accountName | Where-Object KeyName -eq $ActiveCredentialId).value
        $credentialValue = "DefaultEndpointsProtocol=https;AccountName=$accountName;AccountKey=$accountKey;EndpointSuffix=$storageEndpointSuffix"
...
# Function to regenerate a credential (account connection string) via the secret provider (Azure Storage).
...
function Invoke-CredentialRegeneration([string]$InactiveCredentialId, [string]$ProviderAddress) {
    ...
    # Attempt to regenerate the inactive credential (Azure Storage account connection string) and return it.
    try {
        $null = New-AzStorageAccountKey -ResourceGroupName $resourceGroupName -Name $accountName -KeyName $InactiveCredentialId
        $accountKey = (Get-AzStorageAccountKey -ResourceGroupName $resourceGroupName -AccountName $accountName | Where-Object KeyName -eq $InactiveCredentialId).value
        $credentialValue = "DefaultEndpointsProtocol=https;AccountName=$accountName;AccountKey=$accountKey;EndpointSuffix=$storageEndpointSuffix"
        return @($credentialValue, $null)
    }
    catch [Microsoft.Rest.Azure.CloudException] {

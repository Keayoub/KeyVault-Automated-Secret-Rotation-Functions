<#
This PowerShell script is designed for Azure Functions that automatically handle the rotation and import of credentials (Azure Cosmos DB read-only account keys) stored in Azure Key Vault (AKV) by responding to Azure Event Grid events.
...
# Azure Cosmos DB accounts support two read-only access keys, and this function switches between them (either 'PrimaryReadonlyMasterKey' or 'SecondaryReadonlyMasterKey').
...
# Function to retrieve the value of the active credential (read-only account key) from the secret provider (Azure Cosmos DB).
# This function validates the input and retrieves the specified read-only key from the Azure Cosmos DB account.
function Get-CredentialValue([string]$ActiveCredentialId, [string]$ProviderAddress) {
    ...
    # Ensure that the credential ID matches the expected pattern ('PrimaryReadonlyMasterKey' or 'SecondaryReadonlyMasterKey').
    ...
    # Retrieve the specified credential (read-only account key) from the secret provider (Azure Cosmos DB).
    try {
        $credentialValue = (Get-AzCosmosDBAccountKey -ResourceGroupName $resourceGroupName -Name $accountName -Type "ReadOnlyKeys").$ActiveCredentialId
...
# Function to regenerate a credential (read-only account key) via the secret provider (Azure Cosmos DB).
...
function Invoke-CredentialRegeneration([string]$InactiveCredentialId, [string]$ProviderAddress) {
    ...
    # Attempt to regenerate the inactive credential (Azure Cosmos DB read-only account key) and return it.
    try {
        $keyKindForRegeneration = Get-KeyKindForRegeneration -CredentialId $InactiveCredentialId
        $credentialValue = New-AzCosmosDBAccountKey -ResourceGroupName $resourceGroupName -Name $accountName -KeyKind $keyKindForRegeneration
        return @($credentialValue, $null)
    }
    catch [Microsoft.Rest.Azure.CloudException] {
        $httpStatusCode = $_.Exception.Response.StatusCode

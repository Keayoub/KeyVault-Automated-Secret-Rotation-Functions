<#
This PowerShell script is designed for Azure Functions that automatically handle the rotation and import of credentials (Azure Cosmos DB read-write account keys) stored in Azure Key Vault (AKV) by responding to Azure Event Grid events.
...
# Function to retrieve the value of the active credential (read-write account key) from the secret provider (Azure Cosmos DB).
# This function validates the input and retrieves the specified read-write key from the Azure Cosmos DB account.
function Get-CredentialValue([string]$ActiveCredentialId, [string]$ProviderAddress) {
    ...
    # Retrieve the specified credential (read-write account key) from the secret provider (Azure Cosmos DB).
    try {
        $credentialValue = (Get-AzCosmosDBAccountKey -ResourceGroupName $resourceGroupName -Name $accountName -Type "Keys").$ActiveCredentialId
...
# Function to regenerate a credential (read-write account key) via the secret provider (Azure Cosmos DB).
...
function Invoke-CredentialRegeneration([string]$InactiveCredentialId, [string]$ProviderAddress) {
    ...
    # Attempt to regenerate the inactive credential (Azure Cosmos DB read-write account key) and return it.
    try {

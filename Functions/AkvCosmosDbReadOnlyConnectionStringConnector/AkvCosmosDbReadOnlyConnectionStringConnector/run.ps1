<#
This PowerShell script is designed for Azure Functions that automatically handle the rotation and import of credentials (Azure Cosmos DB read-only account connection strings) stored in Azure Key Vault (AKV) by responding to Azure Event Grid events.
...
# Function to retrieve the value of the active credential (read-only account connection string) from the secret provider (Azure Cosmos DB).
# This function validates the input and retrieves the specified read-only connection string from the Azure Cosmos DB account.
function Get-CredentialValue([string]$ActiveCredentialId, [string]$ProviderAddress) {
    ...
    # Retrieve the specified credential (read-only account connection string) from the secret provider (Azure Cosmos DB).
    try {
...
# Function to regenerate a credential (read-only account connection string) via the secret provider (Azure Cosmos DB).
...
function Invoke-CredentialRegeneration([string]$InactiveCredentialId, [string]$ProviderAddress) {
    ...
    # Attempt to regenerate the inactive credential (Azure Cosmos DB read-only account connection string) and return it.
    try {
